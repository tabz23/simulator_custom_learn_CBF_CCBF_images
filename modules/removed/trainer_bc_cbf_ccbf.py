...
CCBF_CHECKPOINT = "/Users/i.k.tabbara/Documents/python directory/ccbf_checkpoint_3999.pt"
...
def main():
    ...
    print("Evaluating nominal CBF-QP controller (goal-reaching)...")
    print("="*50)
    cbf_qp_metrics_goal_reaching = evaluate_cbf_qp_controller(cbf_qp_controller_goal_reaching, env, num_episodes=100, render=False, render_delay=0.00001)
    ...
    print("Evaluating CBF-QP controller (BC + CBF)...")
    print("="*50)
    cbf_qp_metrics_bc = evaluate_cbf_qp_controller(cbf_qp_controller_bc, env, num_episodes=100, render=False, render_delay=0.00001)
    ...
    print("="*50)
    cbf_qp_metrics_bc_safe = evaluate_cbf_qp_controller(cbf_qp_controller_bc_safe, env, num_episodes=100, render=False, render_delay=0.00001)
    ...
    # CBF-QP controller for goal-reaching with behavior cloning model
    ccbf_qp_controller_goal_reaching = CBFQPController_goal_reaching(bc_model, ccbf_model, device=DEVICE, env=env,alpha=0.5)
    print("\n" + "="*50)
    print("Evaluating nominal CCBF-QP controller (goal-reaching)...")
    print("="*50)
    ccbf_qp_metrics_goal_reaching = evaluate_cbf_qp_controller(ccbf_qp_controller_goal_reaching, env, num_episodes=100, render=False, render_delay=0.00001)

    # CBF-QP controller using BC model and CBF
    ccbf_qp_controller_bc = CBFQPController_BC(bc_model, ccbf_model, device=DEVICE,alpha=0.5)
    print("\n" + "="*50)
    print("Evaluating CCBF-QP controller (BC + CCBF)...")
    print("="*50)
    ccbf_qp_metrics_bc = evaluate_cbf_qp_controller(ccbf_qp_controller_bc, env, num_episodes=100, render=False, render_delay=0.00001)

    # NEW: CBF-QP controller using BC-Safe model and CBF
    ccbf_qp_controller_bc_safe = CBFQPController_BC(bc_safe_model, ccbf_model, device=DEVICE,alpha=0.5)
    print("\n" + "="*50)
    print("Evaluating CCBF-QP controller (BC-Safe + CCBF)...")
    print("="*50)
    ccbf_qp_metrics_bc_safe = evaluate_cbf_qp_controller(ccbf_qp_controller_bc_safe, env, num_episodes=100, render=False, render_delay=0.00001)
    ...
    # Prepare the methods and metrics for comparison
    methods = ["BC (All Data)", "BC-Safe (Safe Data Only)", "CBF-QP (BC + CBF)", "CBF-QP (BC-Safe + CBF)", "CBF-QP (Goal Reaching)", "CCBF-QP controller (goal-reaching)", "CCBF-QP (BC-Safe + CBF)", "CCBF-QP (Goal Reaching)"]
    all_metrics = [bc_metrics, bc_safe_metrics, cbf_qp_metrics_bc, cbf_qp_metrics_bc_safe, cbf_qp_metrics_goal_reaching,ccbf_qp_metrics_goal_reaching, ccbf_qp_metrics_bc, ccbf_qp_metrics_bc_safe]
