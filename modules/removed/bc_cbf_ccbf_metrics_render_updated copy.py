...
CBF_CHECKPOINT  ="/Users/i.k.tabbara/Documents/python directory/best_cbf_VAE_Deterministic_4_cql_final657.pth"
CCBF_CHECKPOINT ="/Users/i.k.tabbara/Documents/python directory/best_cbf_VAE_Deterministic_4_cql_final436.pth"
# CBF_CHECKPOINT="/Users/i.k.tabbara/Documents/python directory/best_cbf_VAE_Deterministic_4_cql_755.pth"##this was used overnight
...
# plt.ion()
# fig, ax = plt.subplots(figsize=(4, 4), facecolor='white')##this is top
...
def main():
    ...
    # print("="*50)
    # bc_metrics = evaluate_model(bc_model, env, num_episodes=30, render=False, render_delay=0.0000000)
    
    # # Evaluate BC-Safe model (on safe data only)
    # print("\n" + "="*50)
    # print("Evaluating BC-Safe model (safe data only)...")
    # print("="*50)
    # bc_safe_metrics = evaluate_model(bc_safe_model, env, num_episodes=30, render=False, render_delay=0.0000000)


####UNCOMMENT plt.pause(0.00000000001)# plt.pause(0.00000000001)# plt.pause(0.00000000001) FROM THE CAR ENVIRONMENT RENDER IN ORDER TO RENDER
    ...
    print("="*50)
    cbf_qp_metrics_goal_reaching = evaluate_cbf_qp_controller(cbf_qp_controller_goal_reaching, env, num_episodes=20, render=True, render_delay=0.0000000)
    ...
    print("="*50)
    cbf_qp_metrics_bc = evaluate_cbf_qp_controller(cbf_qp_controller_bc, env, num_episodes=20, render=False, render_delay=0.0000)
    ...
    print("="*50)
    cbf_qp_metrics_bc_safe = evaluate_cbf_qp_controller(cbf_qp_controller_bc_safe, env, num_episodes=20, render=True, render_delay=0.0000)
    ...
    print("="*50)
    ccbf_qp_metrics_goal_reaching = evaluate_cbf_qp_controller(ccbf_qp_controller_goal_reaching, env, num_episodes=20, render=True, render_delay=0.0000)
    ...
    print("="*50)
    ccbf_qp_metrics_bc = evaluate_cbf_qp_controller(ccbf_qp_controller_bc, env, num_episodes=20, render=True, render_delay=0.0000)
    ...
    print("="*50)
    ccbf_qp_metrics_bc_safe = evaluate_cbf_qp_controller(ccbf_qp_controller_bc_safe, env, num_episodes=20, render=True, render_delay=0.0000)
...
# Prepare the methods and metrics for comparison
    ...
    with open("render.txt", "a") as f:
        ...
        # Write header
        f.write(f"{'Method':<35} {'Avg Reward':<15} {'Avg Cost':<15} {'Success Rate':<15} {'Collision Rate':<15}\n")
        
        # Print and write the metrics for each method
        # print("CBF_CHECKPOINT: ", CBF_CHECKPOINT)
        # print("CCBF_CHECKPOINT: ", CCBF_CHECKPOINT)
        ...
        for method, metrics in zip(methods, all_metrics):
            if metrics is not None:
                avg_reward, avg_cost, success_rate, collision_rate = metrics
                result_line = f"{method:<35} {avg_reward:<15.2f} {avg_cost:<15.2f} {success_rate:<15.1f} {collision_rate:<15.1f}\n"
            else:
                result_line = f"{method:<35} {'N/A':<15} {'N/A':<15} {'N/A':<15} {'N/A':<15}\n"
            
            # Print and append to the file
