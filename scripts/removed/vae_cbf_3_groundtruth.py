...
##CBF FOR GROUND TRUTH STATES. WORKS WELL THIS WITHOUT USING LATENT STATE AND DYNAMICS
# def compute_cbf_loss(cbf_model, dynamics_model, batch, device,
#                      safe_distance=4, eps_safe=0.1, eps_unsafe=0.2,
...
#     for t in range(1, seq_length):###PRINT OUT SHAPES HERE TO CHECK THEM
...
#         # ground_truth_state = ground_truth_state.detach().clone().requires_grad_(True)####imp note: z_current is not a leaf tensor, meaning it has been derived from another tensor via some computation. In PyTorch, only leaf tensors (those created directly, not as the result of an operation) can have requires_grad set manually.
#         # B = cbf_model(ground_truth_state).reshape(-1, 1)
#         # grad_b = torch.autograd.grad(B, ground_truth_state, grad_outputs=torch.ones_like(B), retain_graph=True)[0]
...
#         # Compute metrics
#         # safe_correct += ((B > 0) == is_safe.reshape(-1,1)).sum().item()  before was
...
def main():
    ...
    # Create dataset
    full_dataset = CombinedDataset(data, sequence_length=100, prediction_horizon=100)
    ...
    for epoch in trange(epochs):
        ...
        # Validation (CBF) #commenting this bc there is an error:
    '''
          File "/Users/i.k.tabbara/Documents/python directory/simulator_custom_learn_CBF_CCBF_images/scripts/vae_cbf_3.py", line 441, in main
    val_loss_cbf = compute_cbf_loss(cbf_model, dynamics_model, batch, device,
  File "/Users/i.k.tabbara/Documents/python directory/simulator_custom_learn_CBF_CCBF_images/scripts/vae_cbf_3.py", line 240, in compute_cbf_loss
