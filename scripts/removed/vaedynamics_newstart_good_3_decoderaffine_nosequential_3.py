...
def train(vae, dynamics_model, train_loader, val_loader, vae_optimizer, dynamics_optimizer, device, epochs=100,
    ...
    wandb.init(project="vae-dynamics-model", name="training_run_with_recon_loss")

    for epoch in trange(epochs):
        vae.train()
        dynamics_model.train()
        total_vae_loss = 0.0
        total_latent_dynamics_loss = 0.0  # Renamed for clarity
        total_recon_dynamics_loss = 0.0   # Renamed for clarity
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            latent_dynamics_loss_seq = 0.0    # Renamed for clarity
            recon_dynamics_loss_seq = 0.0     # Renamed for clarity
        ...
        total_val_latent_dynamics_loss = 0.0    # Renamed for clarity
        total_val_recon_dynamics_loss = 0.0     # Renamed for clarity
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(val_loader):
                ...
                val_latent_dynamics_loss_seq = 0.0    # Renamed for clarity
                val_recon_dynamics_loss_seq = 0.0     # Renamed for clarity
        ...
        if combined_val_loss < best_val_loss:
            ...
            torch.save(vae.state_dict(), 'vae_model_final_3.pth')
            torch.save(dynamics_model.state_dict(), 'dynamics_model_final_3.pth')
...
def main():
    ...
    # Save final model
    torch.save(trained_vae.state_dict(), 'final_vae_model_3.pth')
    torch.save(trained_dynamics.state_dict(), 'final_dynamics_model_3.pth')
