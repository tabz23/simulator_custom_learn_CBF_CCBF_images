...
class DeterministicDynamicsModel(nn.Module):
    ...
    # Generate new latent state
        ...
        # print(prev_latent_state.shape)##torch.Size([32, 4])
        # print(prev_action.shape)##torch.Size([32, 2])
    ...
    ##this was before, i think its wrong because of the way we compte predicted_next_latent_state using xk and uk-1. we should use xk-1 and uk-1 to predict xk using the dynamics instead and then calculate loss wstate by subtracting xk gotten from decoder (Ik, uk-1,xk-1) using encoder and xk using dynamics where its dynamics(xk-1,uk-1)
    #     new_latent_state = self.encoder(current_obs, prev_latent_state, prev_action)
    #     # Reconstruct observation
    #     reconstructed_obs = self.decoder(new_latent_state)    
    #     # Predict next latent state (using AffineDynamics)
    #     predicted_next_latent_state = self.dynamics_predictor.forward_next_state(new_latent_state, prev_action)  
