...
def compute_losses(model, current_state, z_prev, prev_action, device):
    ...
    recon_loss1 = 1 - ssim(predicted_recon,current_state)
    ...
    return {
        ...
        'z_predicted': z_predicted.detach(),  # Added to return predicted latent state
        'x_recon': x_recon_current.detach(),
        'predicted_recon': predicted_recon.detach()  # Added to return reconstructed image from predicted latent
    }
...
def visualize_sequence_with_losses(model, sequence_data, device, save_path="sequence_visualization.png"):
    ...
    # Storage for reconstructed images, predicted reconstructions, and losses
    ...
    for t in range(seq_length):
        ...
        current_latent_states.append(losses['z_current'].cpu().numpy())
        predicted_latent_states.append(losses['z_predicted'].cpu().numpy())
        
        # Store reconstructed states
        reconstructed_states.append(losses['x_recon'].cpu())
        predicted_reconstructions.append(losses['predicted_recon'].cpu())
    ...
    # Print latent state comparison
    print("\n===== Latent State Comparison =====")
    for t in range(seq_length):
        print(f"\nStep {t+1}:")
        print(f"  Current Latent (z_current): {current_latent_states[t].flatten()}")
        print(f"  Predicted Latent (z_predicted): {predicted_latent_states[t].flatten()}")
        
        # Calculate euclidean distance between the latent states
        euclidean_dist = np.linalg.norm(current_latent_states[t] - predicted_latent_states[t])
        print(f"  Euclidean Distance: {euclidean_dist:.6f}")
    
    # Create the figure for visualization
    fig = plt.figure(figsize=(20, 9))  # Increased height for 3 rows
    ...
    for t in range(seq_length):
        ...
        # Reconstructed image from z_current in middle row
        ax2 = fig.add_subplot(gs[1, t])
        ax2.imshow(tensor_to_img(reconstructed_states[t]))
        ax2.set_title(f"Recon: z_current\nLoss: {all_losses[t]['recon_loss2']:.4f}")
        ax2.axis('off')
        
        # Reconstructed image from z_predicted in bottom row
        ax3 = fig.add_subplot(gs[2, t])
        ax3.imshow(tensor_to_img(predicted_reconstructions[t]))
        ax3.set_title(f"Recon: z_predicted\nLoss: {all_losses[t]['recon_loss1']:.4f}")
        ax3.axis('off')
...
def main():
    ...
    # Load dataset
    data_path = "safe_rl_dataset_images_ALLUNSAFE_big_obstacle.npz"
