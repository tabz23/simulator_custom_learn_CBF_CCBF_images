    ...
    def _create_sequences(self):
        ...
        while i < len(self.dones) - self.sequence_length:
            ...
            if torch.any(sequence_dones[:self.prediction_horizon] == 1):
                #If any timestep in the first prediction_horizon steps has done == 1, it means that the episode ended before the full prediction horizon.
                ##so increment i, and add sequence i->i+sequence length. if any datapt in this sequence has done flag, again increment i.
                i += 1
                continue
...
def compute_cbf_loss(cbf_model, combined_model, batch, device,
                     ...
                     safe_loss_weight=1.0, unsafe_loss_weight=1.3,
    ...
    for t in range(seq_length):
        ...
        if t < seq_length - 1 and safe_mask.sum() > 0:
            ...
            grad_B = torch.autograd.grad(B.sum(), z_current, create_graph=True)[0]##can use b.sum here its like what i did previously with using only B, but doing ones_like(..)
...
def train_cbf(combined_model, cbf_model, train_loader, val_loader, optimizer, device, 
    ...
    for epoch in trange(epochs, desc="Training CBF"):
        ...
        if epoch_val_loss < best_val_loss:
            ...
            torch.save(cbf_model.state_dict(), 'best_cbf_VAE_Deterministic_4.pth')
    ...
    # Save final model
    torch.save(cbf_model.state_dict(), 'final_cbf_model_deterministic.pth')
...
def main():
    ...
    # Initialize wandb
    wandb.init(project="cbf-training-latent-dynamics", name="cbf-training-latent-space_deterministic")
    
    # Parameters and configuration
    config = {
        # Dataset parameters
        'dataset_path': "safe_rl_dataset_images_ALLUNSAFE_big_obstacle_3000traj.npz",
        'sequence_length': 6,
    ...
    try:
        ...
        vae.load_state_dict(torch.load('vae_model_final_5.pth', map_location=device))##LOADED DETERMINISTIC VAE
        dynamics_model.load_state_dict(torch.load('dynamics_model_final_5.pth', map_location=device))##LOADED DYNAMICS
