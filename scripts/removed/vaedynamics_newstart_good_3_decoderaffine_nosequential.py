    ...
    def train(vae, dynamics_model, train_loader, val_loader, vae_optimizer, dynamics_optimizer, device, epochs=100,
        ...
        for epoch in trange(epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                # Predict next latent state: ONLY using the FIRST state and action in sequence
                # First, encode the first state in the sequence
                first_state = states[:, 0, :, :, :]
                _, _, _, current_latent = vae(first_state)  # Shape: [batch_size, latent_dim]
                # Get the action corresponding to the first state
                first_action = actions[:, 0]  # Shape: [batch_size, action_dim]

                # Next encode the second state in the sequence which we want to PREDICT
                second_state = states[:, 1, :, :, :]
                _, _, _, next_latent_gt = vae(second_state)  # Shape: [batch_size, latent_dim]

                # Predict the next latent state using dynamics
                predicted_next_latent = dynamics_model(current_latent, first_action)  # Shape: [batch_size, latent_dim]

                loss_dynamics = F.mse_loss(predicted_next_latent, next_latent_gt)
                loss_dynamics = dynamics_weight * loss_dynamics
                loss_dynamics.backward()
                dynamics_optimizer.step()
                total_dynamics_loss += loss_dynamics.item()

            # Validation (Simplified, only VAE validation)
            vae.eval()
            total_val_loss = 0.0
            with torch.no_grad():
                for batch_idx, batch in enumerate(val_loader):
                    ...
                    for t in range(states.shape[1]):  # Iterate through the sequence length
                        ...
                        recon_x, mu, logvar, _ = vae(current_state)
                        loss_vae = vae_loss(recon_x, current_state, mu, logvar)
                        val_loss_seq += loss_vae.item()
                        

                    total_val_loss += val_loss_seq / states.shape[1]
            ...
            wandb.log({
                ...
                'val_vae_loss': avg_val_loss
            })

            # Model checkpoint (Simplified, only VAE checkpoint)
            if avg_val_loss < best_val_loss:
                best_val_loss = avg_val_loss
                torch.save(vae.state_dict(), 'vae_model_final.pth')
                torch.save(dynamics_model.state_dict(), 'dynamics_model_final.pth')
                print("saved VAE and Dynamics model")

            print(
                f"Epoch {epoch + 1}: Train VAE Loss {avg_train_vae_loss:.4f}, Train Dynamics Loss {avg_train_dynamics_loss:.4f}, Val VAE Loss {avg_val_loss:.4f}")
    ...
    def main():
        ...
        # Save final model
        torch.save(trained_vae.state_dict(), 'final_vae_model.pth')
        torch.save(trained_dynamics.state_dict(), 'final_dynamics_model.pth')
