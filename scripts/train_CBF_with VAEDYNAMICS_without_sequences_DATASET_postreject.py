...
class CombinedDataset(Dataset):
    def __init__(self, data):
        """
        Initialize dataset without sequence handling
        Args:
...
def compute_cbf_loss(cbf_model, combined_model, batch, device,
    ...
    """Compute the CBF loss with added CQL-inspired loss, modified for non-sequential data."""
    ...
    if safe_mask.sum() > 0:
        ...
        loss_grad_vector_safe_action = gradient_loss_weight * F.relu(eps_grad - cbf_condition) * safe_mask_grad
        loss_grad_vector_unsafe_action = (gradient_loss_weight/2) * F.relu(eps_grad + cbf_condition) * unsafe_mask_grad
        ...
        gradient_loss = loss_grad_safe + loss_grad_unsafe
...
def train_cbf(combined_model, cbf_model, train_loader, val_loader, optimizer, device, 
              ...
              use_cql_actions=True, cql_actions_weight=0.1,
              num_action_samples=10, temp=0.7, detach=False,eps_grad=0.02):
    """
    Train CBF model using pre-trained VAE and dynamics models - modified for non-sequential data
    """
    ...
    for epoch in trange(epochs, desc="Training CBF"):
        ...
        for batch_idx, batch in enumerate(tqdm(train_loader, desc=f"Epoch {epoch+1} Training")):
            ...
            loss, metrics = compute_cbf_loss(
                ...
                num_action_samples=num_action_samples, temp=temp, detach=detach,eps_grad=eps_grad
            )
...
def parse_args():
    ...
    # CBF parameters
    parser.add_argument("--safe_distance", type=float, default=7.0, help="Safe distance threshold")
    parser.add_argument("--eps_safe", type=float, default=0.08, help="Epsilon safe threshold")
    parser.add_argument("--eps_unsafe", type=float, default=0.15, help="Epsilon unsafe threshold")
    parser.add_argument("--gradient_loss_weight", type=float, default=2.0, help="Gradient loss weight")
    parser.add_argument("--eps_grad", type=float, default=0.02, help="Gradient loss weight")
    # Checkpointing
...
def main():
    ...
    # Initialize wandb
    wandb.init(project="cbf-training-latent-dynamics", 
               name=f"cbf-training-nonsequence-deterministic-with-cql_{randomnb}")
    ...
    full_dataset = CombinedDataset(data)
