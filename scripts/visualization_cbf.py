...
def visualize_trajectories_and_cbf(data, cbf_paths, resolution=100, device="mps"):
    """
    ...
    obstacle = plt.Circle((obstacle_position[0], obstacle_position[1]), 4.0, color='gray', alpha=0.5)
    ...
    ax_traj.set_title("Density of All Car Trajectories")
    ...
    for i, (cbf, name) in enumerate(zip(cbf_models, cbf_names)):
        ...
        im = ax_cbf.imshow(cbf_values, extent=[-20, 25, -20, 25], origin='lower', cmap='coolwarm', aspect='equal')
        plt.colorbar(im, ax=ax_cbf, label=f'CBF Value ({name})')
        ...
        obstacle = plt.Circle((obstacle_pos[0], obstacle_pos[1]), 4.0, color='gray', alpha=0.5)
...
def main():
    ...
    if data is not None:
        ...
        visualize_trajectories_and_cbf(data, [CBF_1,CBF_2,CBF_3,CBF_4,CBF_5], resolution=100)
